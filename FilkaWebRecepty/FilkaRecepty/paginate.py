...
class NewPagePagination(BasePagination):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        self.count = self.get_count(queryset)
        page_size = self.get_page_size(request)
        self.tags_list = self.get_tags_list(queryset)
        self.img_list = self.get_img_list(queryset)
        if not page_size:
            return None
    ...
    def get_tags_list(self, data):
        tags_list = []    
        for food in data:
            for tag in food.foodTags.all():
                id = 0
                for i in tags_list:
                    if i["tag_name"] == tag.foodTag:
                        i["tag_num"] += 1
                        id = 1
                if id == 0:
                    thisdict = dict(tag_name = tag.foodTag, tag_num = 1)
                    tags_list.append(thisdict)
        return tags_list
    
    def get_img_list(self, data):
        img_list=[]
        for food in data:
            for img in food.images.all():
                # id = 0
                # for i in img_list:
                #     if i != img.id:
    ...
    def get_paginated_response(self, data):
        ...
        return Response({
            ...
            'FirstItemsOnPage':self.page.start_index(),
            'LastItemsOnPage':self.page.end_index(),
            'TotalNumOfPages':self.page.paginator.num_pages,
